...
private const val MENU_ITEM_READ_ALOUD = 5
...
private class MyneTextActionModeCallback(
    ...
    var onReadAloudRequested: (() -> Unit)? = null
) : ActionMode.Callback {
    override fun onCreateActionMode(mode: ActionMode?, menu: Menu?): Boolean {
        ...
        onReadAloudRequested?.let {
            menu.add(0, MENU_ITEM_READ_ALOUD, 5, context.getString(R.string.reader_read_aloud))
                .setShowAsAction(MenuItem.SHOW_AS_ACTION_NEVER)
        }
    ...
    override fun onActionItemClicked(mode: ActionMode?, item: MenuItem?): Boolean {
        when (item!!.itemId) {
            ...
            MENU_ITEM_READ_ALOUD -> onReadAloudRequested?.invoke()
            else -> return false
        }
        mode?.finish()
        return true
...
private class MyneSelectionToolbar(
    ...
    private val onReadAloudRequest: ((String) -> Unit)?
) : TextToolbar {
    ...
    override fun showMenu(
        ...
        callback.onReadAloudRequested = {
            val previousClipboard = clipboardManager.primaryClip
            onCopyRequested?.invoke()
            val currentClipboard = clipboardManager.text

            onReadAloudRequest?.invoke(currentClipboard.toString())
...
fun MyneSelectionContainer(
    ...
    onReadAloudRequested: ((String) -> Unit),
