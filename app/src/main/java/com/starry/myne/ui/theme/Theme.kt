...
private fun getColorScheme(
    ...
    if (amoledTheme) {
        val baseDarkScheme = if (materialYouState && Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
            dynamicDarkColorScheme(context)
        } else {
            darkColors
        }

        return baseDarkScheme.copy(surface = Color.Black, background = Color.Black)
    }
 ...
 */
@Composable
fun AdjustEdgeToEdge(activity: AppCompatActivity, themeState: ThemeMode, amoledTheme: Boolean = false) {
    LaunchedEffect(themeState, amoledTheme) {
        if (amoledTheme || themeState == ThemeMode.Dark) {
