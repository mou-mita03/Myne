...
interface LibraryDao {
    ...
    @Query("UPDATE book_library SET is_favorite = :isFavorite WHERE id = :id")
    fun updateFavorite(id: Int, isFavorite: Boolean)

    @Query("UPDATE book_library SET is_favorite = :isFavorite WHERE book_id = :bookId")
    fun updateFavoriteByBookId(bookId: Int, isFavorite: Boolean)

    @Query("UPDATE book_library SET is_favorite = 0")
    fun clearFavorites()
    ...
    @Query("SELECT * FROM book_library ORDER BY created_at DESC")
    fun getAllItemsList(): List<LibraryItem>
    ...
    @Query(
        "UPDATE book_library SET title = :title, authors = :authors, file_path = :filePath, " +
            "created_at = :createdAt WHERE book_id = :bookId"
    )
    fun updateBookFile(
        bookId: Int,
        title: String,
