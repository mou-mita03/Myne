...
fun List<ReaderItem>.asPlainText(): String {
    return buildString {
        this@asPlainText.forEach { item ->
            val text = when (item) {
                is ReaderItem.Text -> item.spans.asAnnotatedString().text
                is ReaderItem.Blockquote -> item.spans.asAnnotatedString().text
                is ReaderItem.CodeBlock -> item.code.asAnnotatedString(collapseWhitespace = false).text
                is ReaderItem.Image -> ""
            }.trim()
